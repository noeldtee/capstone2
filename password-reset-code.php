...
function send_password_reset($firstname, $email, $reset_token) {
    try {
        ...
        <p>Hi {$firstname}, We received a request to reset your password. Please click the link below to reset it:</p>
        <br><br>
        <a href='http://localhost/capstone-admin/reset-password.php?token={$reset_token}'>Reset Password</a>
...
if (isset($_POST['password_update'])) {
    ...
    if (!$conn || !$conn->ping()) {
        $conn = mysqli_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_DATABASE); // Reconnect
        if (!$conn) {
            redirect("reset-password.php?token={$reset_token}&email={$email}", 'Database connection failed. Please try again later.', 'danger');
            exit();
        }
        mysqli_set_charset($conn, 'utf8mb4'); // Set charset after reconnect
    }
    ...
    if ($result && mysqli_num_rows($result) == 1) {
        ...
        $now = new DateTime();
        if ($now > $expires_at) {
            redirect("reset-password.php?token={$reset_token}&email={$email}", 'Password reset link has expired. Please request a new link.', 'danger');
            exit();
        }

        // Only check new_password and confirm_password for emptiness, as email and token are from hidden fields
        if (empty($new_password) || empty($confirm_password)) {
            redirect("reset-password.php?token={$reset_token}&email={$email}", 'New Password and Confirm Password are required.', 'danger');
            exit();
        }

        // Validate password match
        if ($new_password !== $confirm_password) {
            redirect("reset-password.php?token={$reset_token}&email={$email}", 'Password and Confirm Password do not match.', 'danger');
            exit();
        }
        ...
        if ($stmt->affected_rows > 0) {
            ...
            redirect("reset-password.php?token={$reset_token}&email={$email}", 'Did not update password. Something went wrong.', 'danger');
        }
    } else {
        redirect('password-reset.php', 'Invalid token.', 'danger');
    }
